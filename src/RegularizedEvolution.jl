...
@from "Heap.jl" import heapify_trees
    ...
    if options.cuda
        ...
        @inbounds Threads.@threads for i=1:n_evol_cycles
            ...
            babies[i] = nextGeneration(dataset, baseline, allstar, temperature,
                                       ...
                                       delayEvaluation=true)
        end
        trees = [babies[i].tree for i=1:n_evol_cycles]
        heaps = heapify_trees(trees)
        outputs = options.cuda_evaluator(1, heaps, dataset.X) #TODO: Add batched options
